...
// from keywords to take in account.
...
// TODO DONE : Modify this function to pass the keyword in lowercase and remove special characters
const cleanedKeyword = (keyword) => {
  let cleanedKeyword = keyword;
  
  // It's my codes
  cleanedKeyword = cleanedKeyword.toLowerCase().replace(/[\W_]+/g, '');
  // ---------------------------------
...
// TODO DONE : Modify this function to return the articles containing at leat one of the selected keywords.
const reloadArticles = () => {
  ...
  if (currentKeywords.length == 0)
    ...
    articlesToShow = data.articles.filter(article => {
      return (article.tags.reduce((acc, tag) => acc || currentKeywords.includes(tag), false));
    });
  // ---------------------------------
...
// TODO DONE : Modify this function to show the keyword containging a part of the word inserted into the form (starting autocompletion at 3 letters)
// TODO DONE : We also show all the words from the same category than this word.
// TODO DONE : We show in first the keyword containing a part of the word inserted.
// TODO DONE: If a keyword is already in the list of presents hashtags (checkbox list), we don't show it
const showKeywordsList = (value) => {
  ...
  if (value.length >= 3) {
    ...
    // It's my codes
    // proposedKeywords = keywordsCategories.filter(category => {
    //   return category.keywords.reduce((acc, item) => acc || item.includes(value), false);
    // })[0].keywords;
    
    let selectedCategories = keywordsCategories.reduce((acc, category) => {
      let test = category.keywords.reduce((acc, keyword) => acc || keyword.includes(value), false);
      if (test)
      return [...acc, category];
      else
      return acc;
    ...
    proposedKeywords = selectedCategories.map(category => category.keywords).flat();
    proposedKeywords =  proposedKeywords.reduce((acc, item) => item.includes(value) ? [item, ...acc] : [...acc, item], []);
    proposedKeywords = proposedKeywords.filter(keyword => !keywords.includes(cleanedKeyword(keyword)));
    // ---------------------------------
